...
tensoesfaults =pd.read_csv("1khz/tensoesfault.csv",sep=",")
correntesfaults = pd.read_csv("1khz/correntesfault.csv",sep=",")

res =14             #best 10
window_size = 10    #best 5
addressSize = 11    #best 10
...
Ioffset     =+0.2      # A corrente tem amplitude maxima de 0.4 e oscila a baixo de 0

meanvalidation =10

def func(x,tensoes,correntes,tensoesfaults,correntesfaults):
    res = int(x[0])
    window_size = int(x[1])
    addressSize = int(x[2])
    ...
    cpl_V_fault_Y_             = ((copy.copy(tensoesfaults.values[:, 2]) / Vbase) + Voffset)[:-3]
    dcstatcom_I_faults_Y_      = ((copy.copy(correntesfaults.values[:, 1]) / Ibase) + Ioffset)[:-3]
    grid_I_faults_Y_           = ((copy.copy(correntesfaults.values[:, 4]) / Ibase) + Ioffset)[:-3]
    ...
    cpl_V_fault = copy.copy(cpl_V_fault_Y_)
    dcstatcom_I_faults = copy.copy(dcstatcom_I_faults_Y_)
    grid_I_faults = copy.copy(grid_I_faults_Y_)
    ...
    for i in range(len(gray_time)):
        ...
        if(i<len(cpl_V_fault)):
            cpl_V_fault[i]          = dec2gray(cpl_V_fault[i], res)
            dcstatcom_I_faults[i]   = dec2gray(dcstatcom_I_faults[i], res)
            grid_I_faults[i]        = dec2gray(grid_I_faults[i], res)
    ...
    cpl_V_fault_tapped       = rolling_window(cpl_V_fault,window_size)
    grid_I_faults_tapped       = rolling_window(grid_I_faults,window_size)
    ...
    cpl_V_fault_tapped_wp = np.empty(shape=[len(cpl_V_fault_tapped)],dtype=object)
    grid_I_faults_tapped_wp = np.empty(shape=[len(grid_I_faults_tapped)],dtype=object)


    input_wp = np.empty(shape=[len(gray_grid_I_tapped)],dtype=object)
    input_fault_wp = np.empty(shape=[len(cpl_V_fault_tapped)],dtype=object)
        ...
        for j in range(len(gray_grid_I_tapped[0])):
            if j==0:
               ...
               # gray_dcstatcom_I_tapped_wp[i] = np.array([int(d) for d in str(int(gray_dcstatcom_I_tapped[i][j])).zfill(res)])
                ...
                if(i<len(cpl_V_fault_tapped)):
                    cpl_V_fault_tapped_wp[i] = np.array([int(d) for d in str(int(cpl_V_fault_tapped[i][j])).zfill(res)])
                    grid_I_faults_tapped_wp[i] = np.array([int(d) for d in str(int(grid_I_faults_tapped[i][j])).zfill(res)])
                    input_fault_wp[i] = np.append(cpl_V_fault_tapped_wp[i], grid_I_faults_tapped_wp[i])
                else:
                    print("endfault")
            else:
                ...
                if (i < len(cpl_V_fault_tapped)):
                    cpl_V_fault_tapped_wp[i] = np.append(cpl_V_fault_tapped_wp[i],[int(d) for d in str(int(cpl_V_fault_tapped[i][j])).zfill(res)])
                    grid_I_faults_tapped_wp[i] = np.append(grid_I_faults_tapped_wp[i],[int(d) for d in str(int(grid_I_faults_tapped[i][j])).zfill(res)])

        input_wp[i] = np.append(input_wp[i], gray_cpl_V_tapped_wp[i])
        input_wp[i] = np.append(input_wp[i], gray_grid_I_tapped_wp[i])
        if (i < len(cpl_V_fault_tapped)):
            input_fault_wp[i] = np.append(input_fault_wp[i], cpl_V_fault_tapped_wp[i])
            input_fault_wp[i] = np.append(input_fault_wp[i], grid_I_faults_tapped_wp[i])


        input_wp[i]=list(input_wp[i])
        if (i < len(cpl_V_fault_tapped)):
            input_fault_wp[i] = list(input_fault_wp[i])
    ...
    input_fault_wp1 = list(input_fault_wp)
    ...
    for k in range(meanvalidation):
        ...
        outfault = rwsd.predict(xpredfaults)
        ...
        desnormalizedout = (np.array(out)-Ioffset)*Ibaseout
        desnormalizedout_wp_test = (np.array(output_wp_test)-Ioffset)*Ibase
        desnormalizedout_fault_wp_test = (np.array(outfault)-Ioffset)*Ibase
...
[ind01out,controlout,error,outhist,errorhist] = func([res,window_size,addressSize],tensoes,correntes,tensoesfaults,correntesfaults)
